...
headrow[i].sorttable_columnindex=i;headrow[i].sorttable_tbody=table.tBodies[0];dean_addEvent(headrow[i],"click",sorttable.innerSortFunction=function(e){placeClass(!0);if(this.className.search(/\bsorttable_sorted\b/)!=-1){this.classList.remove('sorttable_sorted');this.classList.remove('sorted');this.removeChild(document.getElementById('sorttable_sortfwdind'));sorttable.innerSortFunction.apply(document.getElementById('sort_rank'),[]);return}
theadrow=this.parentNode;forEach(theadrow.childNodes,function(cell){if(cell.nodeType==1){cell.classList.remove('sorttable_sorted_reverse');cell.classList.remove('sorttable_sorted')}});sortfwdind=document.getElementById('sorttable_sortfwdind');if(sortfwdind){sortfwdind.parentNode.removeChild(sortfwdind)}
sortrevind=document.getElementById('sorttable_sortrevind');if(sortrevind){sortrevind.parentNode.removeChild(sortrevind)}
this.classList.add('sorttable_sorted');this.classList.add('sorted');sortfwdind=document.createElement('span');sortfwdind.id="sorttable_sortfwdind";sortfwdind.innerHTML=stIsIE?'&nbsp<font face="webdings">5</font>':'&nbsp;&#x25BE;';this.appendChild(sortfwdind);row_array=[];col=this.sorttable_columnindex;rows=this.sorttable_tbody.rows;for(var j=0;j<rows.length;j++){row_array[row_array.length]=[sorttable.getInnerText(rows[j].cells[col]),rows[j]]}
row_array.sort(this.sorttable_sortfunction);if(this.id!="sort_rank"){row_array.reverse()}
tb=this.sorttable_tbody;for(var j=0;j<row_array.length;j++){tb.appendChild(row_array[j][1])}
delete row_array;placeClass()})}}},guessType:function(table,column){sortfn=sorttable.sort_alpha;for(var i=0;i<table.tBodies[0].rows.length;i++){text=sorttable.getInnerText(table.tBodies[0].rows[i].cells[column]);if(text!=''){if(text.match(/^-?[Ј$¤]?[\d,.]+%?$/)){return sorttable.sort_numeric}
